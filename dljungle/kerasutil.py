...
def save_model_to_hdf5_group(model, f):
    ...
    tempfd, tempfname = tempfile.mkstemp(suffix='.h5', prefix='tmp-kerasmodel')
    try:
        os.close(tempfd)
        ...
        try:
            ...
            for gpu in gpus:
                tf.config.experimental.set_virtual_device_configuration(
                    gpu,
                    [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=int(frac * gpu.memory_limit))])
            print(f"GPU memory usage set to {frac * 100}%")
